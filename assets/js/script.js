...
const themePreviewData = {
    ...
    default: {
        name: 'Default Theme',
        desktop: 'https://via.placeholder.com/800x600/ffffff/666666?text=Default+Desktop+Preview',
        mobile: 'https://via.placeholder.com/400x600/ffffff/666666?text=Default+Mobile+Preview'
    },
    ecommerce: {
        name: 'E-commerce Pro',
        desktop: 'https://via.placeholder.com/800x600/f0f0f0/333333?text=Ecommerce+Desktop+Preview',
        mobile: 'https://via.placeholder.com/400x600/f0f0f0/333333?text=Ecommerce+Mobile+Preview'
    },
    business: {
        name: 'Business Theme',
        desktop: 'https://via.placeholder.com/800x600/e8e8e8/444444?text=Business+Desktop+Preview',
        mobile: 'https://via.placeholder.com/400x600/e8e8e8/444444?text=Business+Mobile+Preview'
    }
};
...
function populateVersionSelect() {
    ...
    if (wizardData.platform && platformData[wizardData.platform]) {
        ...
        versions.forEach(version => {
            ...
            select.append(`<option value="${version}" ${selected}>${version}</option>`);
        });
    }
}
...
function updateSelectedPlatformDisplay() {
    if (wizardData.platform) {
        const platformName = wizardData.platform.charAt(0).toUpperCase() + wizardData.platform.slice(1);
...
function populateFormFields() {
    ...
    if (wizardData.theme) {
        const selectedThemeCard = $(`.theme-card[data-theme="${wizardData.theme}"]`);
...
function updateSummary() {
    ...
    if (wizardData.theme) {
        ...
        $('#summary-theme').html(`<p><strong>Selected Theme:</strong> ${themeName}</p>`);
    }
    ...
    $('#summary-plugins').html(`<p><strong>Enabled Plugins:</strong> ${enabledPlugins || 'None'}</p>`);
}
...
function validateCurrentStep() {
    if (currentStep === 1) {
        ...
        return wizardData.storeInfo && wizardData.storeInfo.name && wizardData.storeInfo.email;
    }
    return true;
}
...
function startInstallation() {
    ...
    $('#nextBtn').prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Installing...');
    ...
    steps.forEach((step, index) => {
        setTimeout(() => {
            ...
            if (index > 0) {
                ...
                prevStep.find('.install-status i').removeClass('fa-spinner fa-spin').addClass('fa-check');
            }
            
            // Activate current step
            const currentStepEl = $(step.selector);
            currentStepEl.addClass('active');
            currentStepEl.find('.install-status i').removeClass('fa-clock').addClass('fa-spinner fa-spin');
            
            // Complete last step
            if (index === steps.length - 1) {
                setTimeout(() => {
                    currentStepEl.removeClass('active').addClass('completed');
                    currentStepEl.find('.install-status i').removeClass('fa-spinner fa-spin').addClass('fa-check');
                    
                    $('#nextBtn').prop('disabled', false).html('<i class="fas fa-external-link-alt"></i> Open Website');
